...
import numpy as np
from losses import *
from utils import generate_game_sample, load_games_list, construct_obs, init_stats, init_weight_four, detach, random_unit_four
...
def main(args):
    ...
    eval_game_list = list(np.arange(0.1, 1, 0.2))
    ...
    for epoch in range(args.epochs):
        if args.batch_size == 1:
            ...
            negative_prob = epoch / args.epochs * 0.1
            if np.random.rand() < negative_prob:
                epsilon = np.random.rand() * (-1)
            else:
                epsilon = np.random.rand()
            loss, _ = loss_four(1, epsilon)
            epoch_w = init_weight_four(args.init_mode)
            for w in epoch_w:
        ...
        if args.use_slow_optimizer and initialized:
            ...
            if args.batch_size == 1:
                slow_epoch_w = init_weight_four(args.init_mode)
            ...
            if args.reg_1:
                if args.batch_size == 1:
                    ...
                    if len(losses_reg_1) > 0:
                        loss_reg_1 = torch.mean(losses_reg_1)
                    else:
                        loss_reg_1 = 0
    ...
    for idx, loss_line in enumerate(game_list):
        ...
        for wi in tqdm(initial_w):
            ...
            while count < 1000:
                ...
                for j in range(args.n_player):
                    para_idx = j 
                    players_w[j] = players_w[j] - (update[para_idx, 0] * grads[para_idx] - update[para_idx, 1] * Ate[para_idx] - update[para_idx, 2] * Ss[para_idx])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_player', type=int, default=4)
