    ...
    for epoch in range(args.epochs):
        ...
        for iterations in range(args.inner_iterations):
            if args.batch_size == 1:
                ...
                Sg = torch.cat(Sg)
            obs = []
            if 'grad' in args.formula:
                obs.append(grads.view(-1, 1))
            if 'A' in args.formula:
                obs.append(Ag.view(-1, 1))
            if 'S' in args.formula:
                obs.append(Sg.view(-1, 1))        
            # obs = [grads.view(-1, 1)]            
            obs = torch.cat(obs, 1)
            if iterations == 0:
            ...
            if args.batch_size == 1:
                ...
                for idx, epoch_w in enumerate(epoch_ws):
                    for j in range(args.n_player):
                        ...
                        if args.direct:
                            epoch_w[j] = epoch_w[j] - update[para_idx, 0] 
                        else:
                            if 'grad' in args.formula:
                                epoch_w[j] = epoch_w[j] - update[para_idx, 0] * grads[para_idx] 
                            if 'A' in args.formula:
                                epoch_w[j] = epoch_w[j] + update[para_idx, 1] * Ag[para_idx]
                            if 'S' in args.formula:
                                epoch_w[j] = epoch_w[j] + update[para_idx, 2] * Sg[para_idx]   
            ...
            if args.normalize_meta_loss:
                ...
                if not args.reg_2:
                    ...
                    step_meta_loss = torch.mean(torch.sum((grads ** 2).view(args.batch_size, -1) * reg_2s.view(args.batch_size, -1), 1)) / 2
                    ...
                    if args.batch_size == 1:
                        ...
                        for idx, slow_epoch_w in enumerate(slow_epoch_ws):
                            for j in range(args.n_player):
                                para_idx = j + args.n_player * idx
                                if 'grad' in args.formula:
                                    slow_epoch_w[j] = slow_epoch_w[j] - slow_update[para_idx, 0] * slow_grads[para_idx] 
                                if 'A' in args.formula:
                                    slow_epoch_w[j] = slow_epoch_w[j] + slow_update[para_idx, 1] * slow_Ag[para_idx]
                                if 'S' in args.formula:
                                    slow_epoch_w[j] = slow_epoch_w[j] + slow_update[para_idx, 2] * slow_Sg[para_idx]   
        ...
        for wi in initial_w:
            ...
            while count < 1000:
                ...
                if 'grad' in args.formula:
                    obs.append(grads.view(-1, 1))
                if 'A' in args.formula:
                    obs.append(Ate.view(-1, 1))
                if 'S' in args.formula:
                    obs.append(Ss.view(-1, 1)) 
                # obs = [grads.view(-1, 1)]
                obs = torch.cat(obs, 1)
                if count == 0:
                    stats = init_stats(obs, feat_levels=levels)
                ...
                for j in range(2):
                    if args.direct:
                        players_w[j] = players_w[j] - update[j, 0] 
                    else:
                        if 'grad' in args.formula:
                            players_w[j] = players_w[j] - update[j, 0] * grads[j] 
                        if 'A' in args.formula:
                            players_w[j] = players_w[j] + update[j, 1] * Ate[j]
                        if 'S' in args.formula:
                            players_w[j] = players_w[j] + update[j, 2] * Ss[j]   
