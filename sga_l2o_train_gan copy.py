...
def main(args):
    ...
    cl = [50, 100, 200, 500, 1000, 2000, 5000, 10000]
    ...
    for epoch in range(args.epochs):
        ...
        for iterations in range(args.inner_iterations):
            ...
            if args.batch_size == 1:
                epoch_w[0] = epoch_w[0] - (update[:epoch_w[0].shape[0],0] * grads[:epoch_w[0].shape[0]].detach() - update[:epoch_w[0].shape[0],1] * Ag[:epoch_w[0].shape[0]] - update[:epoch_w[0].shape[0],2] * Sg[:epoch_w[0].shape[0]]) * scale[0] * 1e-4
                # epoch_w[0] = epoch_w[0] - (update[:epoch_w[0].shape[0],0] * grads[:epoch_w[0].shape[0]]) * scale[0] * 1e-4
                epoch_w[1] = epoch_w[1] - (update[epoch_w[0].shape[0]:,0] * grads[epoch_w[0].shape[0]:].detach() - update[epoch_w[0].shape[0]:,1] * Ag[epoch_w[0].shape[0]:] - update[epoch_w[0].shape[0]:,2] * Sg[epoch_w[0].shape[0]:]) * scale[0] * 1e-4
                # epoch_w[1] = epoch_w[1] - (update[epoch_w[0].shape[0]:,0] * grads[epoch_w[0].shape[0]:]) * scale[0] * 1e-4
            else:
            ...
            if (meta_loss > 100) or (meta_loss < -1e5):
                break
        ...
        if (epoch + 1) % 20 == 0:    
            torch.save({"state_dict": optimizer.state_dict()}, args.output_name)
            if args.cl:
                try:
