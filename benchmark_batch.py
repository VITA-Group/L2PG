    ...
    for idx, loss_line in tqdm(enumerate(lines)):
        ...
        for lr in [args.lr]:
            ...
            for wi in initial_w:
                ...
                count = 0
                mts = [torch.zeros_like(w) for w in players_w]
                vts = [torch.zeros_like(w) for w in players_w]
                while count < 1000:
                    ...
                    elif args.type == 'conopt':
                        players_w[0] = players_w[0] - Ss[0] * args.lamb * lr 
                        players_w[1] = players_w[1] - Ss[1] * args.lamb * lr 
                    elif args.type == 'adam':
                        for m, g in zip(mts, grads):
                            m.data = 0.95 * m.data + 0.05 * g.data
                        for v, g in zip(vts, grads):
                            v.data = 0.99 * v.data + 0.01 * (g.data) ** 2
                        mhat = [m / (1 - 0.95 ** (count + 1)) for m in mts]
                        vhat = [v / (1 - 0.99 ** (count + 1)) for v in vts]
                        players_w[0] = players_w[0] - mhat[0] / torch.sqrt(vhat[0] + 1e-8) * lr 
                        players_w[1] = players_w[1] - mhat[1] / torch.sqrt(vhat[1] + 1e-8) * lr 
