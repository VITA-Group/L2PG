...
def main(args):
    ...
    checkpoints = torch.load('gan_new.pkl_59')
...
def kde(mu, tau, step, bbox=None, xlabel="", ylabel="", cmap='Blues'):
    ...
    plt.savefig(f'l2pg/gen_{step}.png')
...
def evaluate(net, game_list, formula, levels, args, slow=False):
    ...
    if True:
        ...
        for shape in gen_shapes:
            epoch_w[0][cur_sz:cur_sz + shape[0] * shape[1]] = torch.randn(shape[0] * shape[1]) / np.sqrt(shape[0]) * np.sqrt(2) * 0.5
        ...
        for shape in dis_shapes:
            epoch_w[1][cur_sz:cur_sz + shape[0] * shape[1]] = torch.randn(shape[0] * shape[1]) / np.sqrt(shape[0]) * np.sqrt(2) * 0.5
            cur_sz += shape[0] * shape[1]
